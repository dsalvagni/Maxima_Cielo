...
class Maxima_Cielo_PayController extends Mage_Core_Controller_Front_Action
{
	...
	public function verifyAction()
	{
		...
		// 9 cancelada
		// 10 em autenticacao
		Mage::log('Status da CIELO: ' . $status);
		...
		else if($block->getCieloStatus() == 9)
		{
			/**
			 ...
			 */
			Mage::getSingleton('core/session')->addNotice("Você cancelou o pagamento.");
			/**
			 * Redireciona para criar uma nova compra (Recomprar)
			 */
			$this->_redirect('sales/order/reorder/order_id/'.$orderId);
		}
		else if($block->getCieloStatus() == 5)
		{
			 ...
			 * Adiciona uma mensagem (flash)
			 */
			Mage::getSingleton('core/session')->addNotice("O pagamento não foi autorizado.");
			/**
			 * Redireciona para criar uma nova compra (Recomprar)
			 */
			$this->_redirect('sales/order/reorder/order_id/'.$orderId);
